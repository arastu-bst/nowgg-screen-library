...
2026-06-16 — marketing/BluestacksBand.tsx + play/[slug]/page.tsx — added BluestacksBand to the app/play page, as the SAME contained rounded card as the homepage (inside the page's max-w-content column, after Explore by Categories) — Why: designer's "full width banner" meant "the homepage banner, on the app page too," NOT a true edge-to-edge band. First built a `fullBleed` mode (squared, full-width) → designer clarified "like homepage only" → reverted; removed the now-unused fullBleed mode (no dead code). One band treatment, identical on both pages.
